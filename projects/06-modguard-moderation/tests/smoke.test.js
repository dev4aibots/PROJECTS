...
(async () => {
  ...
  // 11. regression: punctuation must not defeat phrase matching.
  // "Congratulations! You won" has to match the lexicon phrase "congratulations you".
  const punct = lib.ruleEngine('Congratulations! You won a crypto giveaway, send 0.1 ETH to claim now');
  const punctFlagged = punct.categories.filter(c => c.flagged).map(c => c.name);
  assert.ok(punctFlagged.includes('scam'), 'punctuated crypto-giveaway scam is detected');
  assert.strictEqual(lib.assembleVerdict(punct).action, 'block');

  // 12. regression: short money/urgency terms are word-boundary matched, so
  // ordinary prose containing "whether"/"method"/"coffee" stays clean.
  const benign = lib.ruleEngine("I'm not sure whether this method of coffee brewing is correct");
  assert.deepStrictEqual(benign.categories.filter(c => c.flagged), [], 'no substring false positives');
  assert.strictEqual(lib.assembleVerdict(benign).action, 'allow');

  // 13. regression: shortener detection still works on raw domains (dots survive).
  const shortened = lib.ruleEngine('URGENT: your account suspended, verify at bit.ly/x2 to avoid fee');
  assert.ok(shortened.categories.find(c => c.name === 'scam').flagged, 'shortener + urgency → scam');
