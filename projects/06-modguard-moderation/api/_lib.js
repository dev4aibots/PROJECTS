...
const MONEY = ['$', 'usd', 'payment', 'money', 'cash', 'crypto', 'bitcoin', 'bank', 'fee', 'deposit', 'refund'];

function scoreLexicon(text, phrases) {
  let hits = 0;
  const matched = [];
  for (const p of phrases) {
    if (text.includes(p)) { hits++; matched.push(p); }
  }
...
function ruleEngine(content) {
  ...
  for (const cat of CATEGORIES) {
    ...
    if (cat === 'scam') {
      ...
      const shortener = SHORTENERS.find(s => text.includes(s));
      const urgency = URGENCY.some(u => text.includes(u));
      const money = MONEY.some(m => text.includes(m));
      if (shortener) {
